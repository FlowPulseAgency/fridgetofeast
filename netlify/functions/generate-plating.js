...
exports.handler = async (event, context) => {
  ...
  if (!auth.isPremium) {
    return {
      statusCode: 403,
      body: JSON.stringify({ error: "premium_required", message: "Upgrade to Premium to unlock Michelin plating instructions!" })
    };
  }
  ...
  try {
    const { recipeTitle, description, ingredients } = JSON.parse(event.body);
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: error.message || "Failed to generate plating instructions." })
    };
  }
};
