...
module.exports = async (req, res) => {
  ...
  if (!auth.isPremium) {
    return res.status(403).json({ error: "premium_required", message: "Upgrade to Premium to unlock Michelin plating instructions!" });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: error.message || "Failed to generate plating instructions." });
  }
};
