...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initSupabase() {
    ...
    const isHosted = host !== 'localhost' && host !== '127.0.0.1' && !host.startsWith('192.168.') && !host.startsWith('10.') && !host.startsWith('172.');
    if (isHosted) {
      try {
  ...
  if (authPasswordToggle && authPasswordInput) {
    authPasswordToggle.addEventListener('click', () => {
      ...
      const icon = authPasswordToggle.querySelector('i');
      if (icon) {
  ...
  const cookbookLoading = document.getElementById('cookbook-loading');
  const cookbookAuthPrompt = document.getElementById('cookbook-auth-prompt');
  const cookbookEmpty = document.getElementById('cookbook-empty');
  const cookbookGrid = document.getElementById('cookbook-grid');
  const cookbookControls = document.getElementById('cookbook-controls');
  const cookbookSearchInput = document.getElementById('cookbook-search-input');
  const cookbookFilterContainer = document.getElementById('cookbook-filter-container');
  ...
  async function handleStripeCallback() {
    ...
    if (stripeStatus === 'success' && sessionId) {
      showLoading("Confirming Premium Upgrade...");
      try {
        const apiPath = getApiPath('verify-checkout');
        ...
        if (!res.ok) {
          throw new Error(verification.error || `Server returned ${res.status}`);
        }
        if (verification.success) {
          if (state.user) {
            state.profile = verification.profile;
          }
          activatePremium();
        ...
        alert("Failed to verify subscription session: " + err.message);
      } finally {
        hideLoading();
      }
    } else if (stripeStatus === 'cancel') {
      alert("Upgrade cancelled. No charges were made.");
  ...
  paywallCheckoutBtn.addEventListener('click', async () => {
    ...
    if (isHostedServer) {
      try {
        const apiPath = getApiPath('create-checkout');
        ...
        alert("Failed to start checkout process. Please try again. Error: " + err.message);
  ...
  if (setupKeyBtn) setupKeyBtn.addEventListener('click', openSettings);
  closeSettingsModal.addEventListener('click', closeSettings);
  settingsOverlay.addEventListener('click', closeSettings);
  cancelSettings.addEventListener('click', closeSettings);
  const toggleDevBtn = document.getElementById('toggle-dev-settings-btn');
  const devContainer = document.getElementById('dev-settings-container');
  const devChevron = document.getElementById('dev-settings-chevron');
  if (toggleDevBtn && devContainer) {
    toggleDevBtn.addEventListener('click', () => {
      const isHidden = devContainer.classList.contains('hidden');
      if (isHidden) {
        devContainer.classList.remove('hidden');
        if (devChevron) devChevron.style.transform = 'rotate(90deg)';
      } else {
        devContainer.classList.add('hidden');
        if (devChevron) devChevron.style.transform = 'rotate(0deg)';
      }
    });
  }
  ...
  analyzeImgBtn.addEventListener('click', async () => {
    ...
    try {
            ...
            contents: [
              {
                parts: [
                  {
                    text: "Identify all specific cooking ingredients in this image. Split your response into: 1) ingredients you are highly confident about, and 2) items you see but are unsure about. Return strictly a JSON object with 'ingredients' (array of strings) and 'clarifications' (array of objects with 'visualAppearance', 'location', and 'question')."
                  },
                  {
                    inlineData: {
      ...
      if (responseData && typeof responseData === 'object' && !Array.isArray(responseData)) {
        ...
        clarificationsArray = responseData.clarifications || [];
      ...
      renderClarifications(clarificationsArray);
    } catch (error) {
  ...
  function renderClarifications(clarifications) {
    const container = document.getElementById('vision-clarifications-container');
    ...
    clarifications.forEach((item, index) => {
      ...
      input.type = 'text';
      input.placeholder = 'Identify this item (e.g. apple, yogurt)...';
      input.style.flex = '1';
      ...
      addBtn.style.padding = '0.4rem 0.75rem';
      ...
      itemRow.appendChild(questionText);
  ...
  generateRecipesBtn.addEventListener('click', async () => {
    ...
    try {
      let isHostedServer = state.isServerMode || (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1');
      let useClientKey = !isHostedServer;
      if (isHostedServer) {
        try {
      ...
      const isAuthNeeded = error.message.includes("unregistered callers") ||
                           error.message.includes("established identity") ||
                           error.message.includes("API Key") ||
                           error.message.includes("401") ||
                           error.message.includes("403");
      if (isAuthNeeded || !state.currentUser) {
        const authModal = document.getElementById('auth-modal');
        if (authModal) authModal.classList.remove('hidden');
        alert("🔑 Account Required:\n\nPlease sign in or create a free account to generate AI recipes!");
      } else {
  ...
  function renderRecipeModalContent() {
      ...
      <div class="recipe-detail-header">
        ...
        <div class="recipe-detail-stats">
          ...
          ${state.isPremium ? `
            ...
            <button id="modal-cookbook-btn" class="btn btn-secondary btn-sm" style="margin-left: 0.5rem; border-color: var(--success); color: var(--success);">
              <i data-lucide="book-open"></i> Save to Cookbook
            </button>
          ` : `
            <button id="modal-cookbook-btn" class="btn btn-secondary btn-sm" style="margin-left: auto; border-color: var(--success); color: var(--success);">
              <i data-lucide="book-open"></i> Save to Cookbook
            </button>
          `}
          ${missingIngredients.length > 0 ? `
            <button id="modal-generate-list" class="btn btn-accent btn-sm" style="margin-left: 0.5rem;">
    ...
    const cookbookBtn = document.getElementById('modal-cookbook-btn');
    if (cookbookBtn) {
      const isAlreadySaved = state.savedRecipes && state.savedRecipes.some(r => r.title === recipe.title);
      if (isAlreadySaved) {
        cookbookBtn.innerHTML = `<i data-lucide="check"></i> Saved to Cookbook`;
        cookbookBtn.style.borderColor = 'var(--success)';
        cookbookBtn.style.color = 'var(--success)';
        cookbookBtn.disabled = true;
      }
      cookbookBtn.addEventListener('click', async () => {
        if (!state.user) {
          alert("Please Sign In first to save recipes to your Cookbook!");
          return;
        }
        cookbookBtn.disabled = true;
        cookbookBtn.textContent = "Saving...";
        try {
          const { data, error } = await supabase
            .from('saved_recipes')
            .insert({
          ...
          if (!state.savedRecipes) state.savedRecipes = [];
          ...
          cookbookBtn.innerHTML = `<i data-lucide="book-open"></i> Save to Cookbook`;
          if (window.lucide) window.lucide.createIcons();
        }
      });
    }
  ...
  async function fetchSavedRecipes() {
    if (!state.user || !supabase) return;
    try {
      if (cookbookLoading) cookbookLoading.classList.remove('hidden');
      if (cookbookGrid) cookbookGrid.classList.add('hidden');
      if (cookbookEmpty) cookbookEmpty.classList.add('hidden');
      if (cookbookAuthPrompt) cookbookAuthPrompt.classList.add('hidden');
      const { data, error } = await supabase
        .from('saved_recipes')
        .select('*')
      ...
      if (cookbookLoading) cookbookLoading.classList.add('hidden');
    }
  }
  function renderCookbookGrid() {
    if (!cookbookView) return;
    if (cookbookLoading) cookbookLoading.classList.add('hidden');
    if (cookbookAuthPrompt) cookbookAuthPrompt.classList.add('hidden');
    if (cookbookEmpty) cookbookEmpty.classList.add('hidden');
    if (cookbookGrid) cookbookGrid.classList.add('hidden');
    ...
    if (!state.user) {
      if (cookbookAuthPrompt) cookbookAuthPrompt.classList.remove('hidden');
      return;
    }
    if (!state.savedRecipes || state.savedRecipes.length === 0) {
      if (cookbookEmpty) cookbookEmpty.classList.remove('hidden');
      return;
    }
    if (cookbookControls) cookbookControls.classList.remove('hidden');
    let filtered = state.savedRecipes || [];
    if (state.cookbookActiveFilter !== 'all') {
      const activeFilter = state.cookbookActiveFilter.toLowerCase();
      filtered = filtered.filter(saved => {
        const recipe = saved.recipe_data;
        if (!recipe.tags) return false;
        return recipe.tags.some(tag => tag.toLowerCase().includes(activeFilter));
      });
    }
    if (state.cookbookSearchQuery.trim() !== '') {
      const q = state.cookbookSearchQuery.toLowerCase();
      filtered = filtered.filter(saved => {
        const recipe = saved.recipe_data;
        const titleMatch = saved.title.toLowerCase().includes(q);
        ...
        const ingMatch = recipe.ingredients && recipe.ingredients.some(ing => ing.name.toLowerCase().includes(q));
    ...
    if (filtered.length === 0) {
      ...
      emptySearchMsg.className = 'glass-panel text-center padding-3';
      emptySearchMsg.style.gridColumn = '1 / -1';
      emptySearchMsg.innerHTML = `
        <i data-lucide="search" style="width: 40px; height: 40px; color: var(--text-muted); margin-bottom: 0.5rem; display: block; margin-left: auto; margin-right: auto;"></i>
        <h4 style="color: #fff; margin-bottom: 0.25rem;">No matching recipes found</h4>
        <p style="color: var(--text-secondary); font-size: 0.9rem;">Try adjusting your search query or category filter!</p>
      `;
      cookbookGrid.appendChild(emptySearchMsg);
    } else {
      filtered.forEach((saved) => {
        ...
        if (state.isPremium) {
          plannerBtnHTML = `
            <button class="btn btn-secondary btn-sm btn-pin-saved" style="padding: 0.65rem; border-color: var(--primary); color: var(--primary);" title="Add to Weekly Planner">
        ...
        card.className = 'recipe-card';
        card.innerHTML = `
          <div class="card-accent-line" style="background: var(--gold); border-radius: var(--radius-sm) var(--radius-sm) 0 0; height: 4px;"></div>
          <div class="recipe-card-body" style="display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
            <div>
              <div class="recipe-meta-row">
                <div class="meta-item">
                  <i data-lucide="clock"></i>
              ...
              <h3 class="recipe-card-title">${saved.title}</h3>
              <p class="recipe-card-desc">${recipe.description || 'Custom saved recipe.'}</p>
            </div>
            <div style="display: flex; gap: 0.5rem; margin-top: 1.25rem; width: 100%;">
              <button class="btn btn-primary btn-sm btn-view-saved" style="flex-grow: 1;">
                <i data-lucide="eye"></i> View Recipe
              </button>
              ${plannerBtnHTML}
              <button class="btn btn-danger btn-sm btn-delete-saved" style="padding: 0.65rem;" title="Delete Recipe">
                <i data-lucide="trash-2"></i>
              </button>
            </div>
          </div>
        `;
        card.querySelector('.btn-view-saved').addEventListener('click', () => {
          openRecipeDetail(recipe);
        });
        if (state.isPremium) {
          const pinBtn = card.querySelector('.btn-pin-saved');
          if (pinBtn) {
        ...
        card.querySelector('.btn-delete-saved').addEventListener('click', async (e) => {
          e.stopPropagation();
          if (confirm(`Are you sure you want to delete "${saved.title}" from your Cookbook?`)) {
    ...
    if (window.lucide) window.lucide.createIcons();
    cookbookGrid.classList.remove('hidden');
  }
  async function deleteSavedRecipe(savedId) {
    if (!supabase) return;
    try {
      const { error } = await supabase
        ...
        .eq('id', savedId);
      if (error) throw error;
      state.savedRecipes = state.savedRecipes.filter(r => r.id !== savedId);
      renderCookbookGrid();
    } catch (err) {
  ...
  const authTrigger = document.querySelector('#cookbook-auth-prompt .btn-auth-trigger');
  if (authTrigger) {
    authTrigger.addEventListener('click', openAuthModal);
  }
  const genTrigger = document.querySelector('#cookbook-empty .btn-generator-trigger');
  if (genTrigger) {
    genTrigger.addEventListener('click', () => switchView('generator'));
  }
  ...
  if (cookbookSearchInput) {
    cookbookSearchInput.addEventListener('input', (e) => {
      state.cookbookSearchQuery = e.target.value;
      renderCookbookGrid();
    });
  }
  if (cookbookFilterContainer) {
    cookbookFilterContainer.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', () => {
        cookbookFilterContainer.querySelectorAll('button').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        state.cookbookActiveFilter = btn.getAttribute('data-filter');
        renderCookbookGrid();
      });
    });
  }
